...
def init_data(
    ...
    try:
        ...
        print("[EYAL]: going to migrate monitoring functions labels")
        _perform_version_10_data_migrations(db = framework.db.sqldb.db.SQLDB(), db_session=db_session)
...
def _migrate_monitoring_functions_labels_v2(db: framework.db.sqldb.db.SQLDB, db_session):
    """
    Update labels for model monitoring infra functions.
    """
    ...
    def add_infra_label(record):
        ...
        # now let's add a new label to the function metadata as well
        function_metadata_labels_dict["newlabel12"] = "newvalue12"
        function_dict["metadata"]["labels"] = function_metadata_labels_dict
        print("[EYAL]: now going to update infra label V2, function metadata after update labels:", function_dict.get("metadata"))
...
def _perform_version_10_data_migrations(
    ...
    _migrate_monitoring_functions_labels_v2(db, db_session)
...
def _migrate_data(
    ...
    while records:
        ...
        if max_iterations is not None and iteration >= max_iterations:
            logger.info("Reached max iterations", {model.__name__})
            break
